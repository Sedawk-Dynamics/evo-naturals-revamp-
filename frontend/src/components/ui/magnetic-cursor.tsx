...
import React, { useRef, useEffect, FC, ReactNode, useSyncExternalStore } from 'react';
import gsap from 'gsap';
// vecteur exports the type as `Vector2` (there is no `Vec2`), so alias it.
import { vec2, type Vector2 as Vec2 } from 'vecteur';
...
export const MagneticCursor: FC<MagneticCursorProps> = ({
  ...
  // Touch detection without setState-in-effect: false on the server,
  // real value on the client (hydration-safe).
  ...
  useEffect(() => {
    ...
    // Magnetic elements are found by event delegation (checked on every
    // pointerover) instead of being collected once on mount. Collected once,
    // any button that renders later — or is re-created, e.g. by a hot reload
    // or a section re-mounting — silently loses the effect until a reload.
    const selector = `[${hoverAttribute}]`;
    const pullers = new WeakMap<HTMLElement, { xTo: gsap.QuickToFunc; yTo: gsap.QuickToFunc }>();
    const pullerFor = (el: HTMLElement) => {
      let puller = pullers.get(el);
      if (!puller) {
        puller = {
          xTo: gsap.quickTo(el, 'x', { duration: 1, ease: 'elastic.out(1, 0.3)' }),
          yTo: gsap.quickTo(el, 'y', { duration: 1, ease: 'elastic.out(1, 0.3)' }),
        };
        pullers.set(el, puller);
      }
      return puller;
    };
    let activeEl: HTMLElement | null = null;

    // Cursor wraps the element
    const enter = (el: HTMLElement) => {
      ...
      gsap.to(cursorEl, {
        ...
        duration: 0.3,
        ease: 'power3.out',
        overwrite: true // true = kill all other tweens on this element ('all' isn't in gsap's types)
      });
    };

    // Cursor shrinks back to a dot; the element springs back into place
    const leave = (el: HTMLElement) => {
      const { xTo, yTo } = pullerFor(el);
      xTo(0);
      yTo(0);
    ...
    // Fires for whatever the pointer just moved onto. Moving between a
    // button's own children finds the same button, so nothing happens.
    const handlePointerOver = (event: PointerEvent) => {
      const el = (event.target as Element).closest<HTMLElement>(selector);
      if (el === activeEl) return;
      // Also covers a hovered element that was removed from the page
      if (activeEl) leave(activeEl);
      activeEl = el;
      if (el) enter(el);
    };

    // Leaving the window entirely (no element under the pointer anymore)
    const handlePointerOut = (event: PointerEvent) => {
      if (!activeEl || event.relatedTarget) return;
      leave(activeEl);
      activeEl = null;
    };

    // Pull the hovered element toward the pointer
    let rafId: number | null = null;
    const handleMagnetMove = (event: PointerEvent) => {
      if (!activeEl || rafId) return;
      rafId = requestAnimationFrame(() => {
        ...
        const { xTo, yTo } = pullerFor(el);
        xTo((event.clientX - (left + width / 2)) * magneticFactor);
        yTo((event.clientY - (top + height / 2)) * magneticFactor);
      });
    };

    document.addEventListener('pointerover', handlePointerOver);
    document.addEventListener('pointerout', handlePointerOut);
    window.addEventListener('pointermove', handleMagnetMove);
    ...
    return () => {
      ...
      window.removeEventListener('pointermove', handleMagnetMove);
      if (rafId) cancelAnimationFrame(rafId);
      if (activeEl) {
        const { xTo, yTo } = pullerFor(activeEl);
        xTo(0);
        yTo(0);
      }
    };
